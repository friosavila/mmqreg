...
\begin{equation}\phantomsection\label{eq-eq1}{q(Y|X,\tau) =X\beta(\tau)
}\end{equation}
...
\begin{equation}\phantomsection\label{eq-eq2}{q(Y|X,\tau) =X\beta+X\gamma(\tau)
}\end{equation}
...
\begin{equation}\phantomsection\label{eq-eq5}{q(Y|X,\tau) =X\beta+q(\varepsilon|\tau) \times X\gamma 
}\end{equation}
...
\begin{equation}\phantomsection\label{eq-eq6}{\beta(\tau) = \beta + q(\varepsilon|\tau) \times \gamma 
}\end{equation}
...
\begin{equation}\phantomsection\label{eq-eqqtile}{\Xi = [ I(k), \hat q(\varepsilon|\tau) \times I(k), \hat \gamma ]
}\end{equation}
...
structure that is more general than the panel structure used in by the
authors. For this excercise, we consider the following data generating
process:
...
and standard deviation 0.5. We assume that there are 50 mutually
...
presents an almost zero bias, even when the samples are is small (about
10 observations per sub-group). Also replicating the results of MSS,
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 18\tabcolsep) * \real{0.0989}}
  >{\centering\arraybackslash}p{(\columnwidth - 18\tabcolsep) * \real{0.0989}}@{}}
\caption{The determinants of government surpluses: Individual Fixed
...
point estimates are identical to the ones reported in \citet{mss2019}
(Table 6), including analytical standard errors (GLS). With our
estimator, however, we are able to also produce both robust and
clustered standard errors for location and scale coefficients. Except
for a few cases, robust and clustered standard errors are larger than
GLS standard errors, which may be an indication of misspecification of
the model. The GLS standard errors we report differ from the ones in MSS
because they use panel standard errors, which are equivalent to our
clustered standard errors, instead of the analytical standard errors we
derive.
...
\subsection{Derivation of the influence
functions}\label{derivation-of-the-influence-functions}

\subsubsection{Model Identification}\label{model-identification}
...
\subsubsection{Estimation of the variance-covariance
...
\paragraph{First Moment Condition: Location
...
\paragraph{Second Moment Condition: Scale
...
\paragraph{Third Moment Condition: Quantile of Standardized
...
\subsubsection{Influence functions}\label{influence-functions}

\paragraph{Location coefficients}\label{location-coefficients}
...
\paragraph{Scale Coefficients}\label{scale-coefficients}
...
\paragraph{Quantile of standardized
...
\tightlist
\item
  \texttt{mmqreg} in Stata: Installed from Github
\item
  \texttt{mmqreg} in R: Programs Available in Github
\item
